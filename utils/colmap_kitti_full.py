...
from utils.kitti_utils import load_camera_info
from utils.data_utils import get_val_frames
from utils.colmap_utils import read_extrinsics_binary, qvec2rotmat
...
def run_colmap_kitti(result, args):
    ...
    for i, image_filename in enumerate(image_filenames):
        ...
        new_basename = f"{basename_no_ext}.png"
        new_image_filename = convert_filename(new_basename)
        c2w_dict[new_image_filename] = c2ws[i]
        mask_image_filenames.append(os.path.join(data_path, 'dynamic_masks/all', new_basename))
    ...
    ret = os.system(feature_extractor_cmd)
    if ret != 0:
        print("Error: COLMAP feature extraction failed")
        return False
    ...
    try:
        ...
        if c.fetchone() is None:
            print("Error: No images table in database")
            conn.close()
            return False
        conn.close()
    ...
    _, extrinsics, _, _ = load_camera_info(args.source_path)
