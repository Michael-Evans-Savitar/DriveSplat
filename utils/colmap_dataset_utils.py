...
def init_filestructure(save_path):
    ...
    sparse_path = save_path / 'sparse/model'
...
def save_pointcloud(imgs, pts3d, msk, sparse_path):
    ...
    pc.export(save_path)
    save_path = sparse_path / 'points3D.bin'
    pc.export(save_path)
...
def save_binary_points3d(pts3d, colors, path):
    points3D_fn = os.path.join(path, 'sparse/model/points3D.bin')
    with open(points3D_fn, 'wb') as f:
