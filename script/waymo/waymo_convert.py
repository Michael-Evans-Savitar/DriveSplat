    ...
    if 'pose' in process_list:
        ...
        for frame_id, frame in tqdm(enumerate(datafile)):
            ...
            for i, camera in enumerate(camera_calibrations):
                ...
                np.savetxt(os.path.join(ego_pose_save_dir, f"{str(frame_id).zfill(6)}_{camera_name - 1}.txt"),
                           camera_pose)
    ...
    if 'lidar' in process_list:
        ...
        for frame_id, frame in tqdm(enumerate(datafile)):
            ...
            for laser_name, laser_name_str in laser_names_dict.items():
                ...
                pcl, pcl_attr = utils.project_to_pointcloud(frame, ri, camera_projection, range_image_pose,
                                                            laser_calibration)
...
def main():
    ...
    parser.add_argument('--process_list', type=str, nargs='+',
                        default=['pose', 'calib', 'image', 'lidar', 'track', 'dynamic_mask'])
    parser.add_argument('--root_dir', type=str, required=True)
    parser.add_argument('--save_dir', type=str, required=True)
    parser.add_argument('--track_file', type=str, default='')
    ...
    for i, scene_id in enumerate(scene_ids_list):
        ...
        print(f'seq_names[i]: {seq_names[i]}')
        print(f'seq_lists[scene_id]: {seq_lists[scene_id]}')
